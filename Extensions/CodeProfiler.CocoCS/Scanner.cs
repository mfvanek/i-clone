...
public class Token {
	...
	public int pos;     // token position by bytes in the source text (starting at 0)
	public int posByChars;  // token position by characters in the source text (starting at 0)
...
public class Buffer {
  ...
  /// </summary>
  /// <param name="beg">start (0-based) in BYTES</param>
  /// <param name="end">end (0-based) in BYTES</param>
  /// <returns></returns>
...
public class Scanner {
	...
	int posByChars;   // position by unicode characters starting with 0
	...
	bool Comment0() {
		int level = 1, pos0 = pos, line0 = line, col0 = col, posByChars0 = posByChars;
		NextCh();
		if (ch == '/') {
			NextCh();
			for(;;) {
			...
			buffer.Pos = pos0; NextCh(); line = line0; col = col0; posByChars = posByChars0;
		}
		return false;
	}
