...
// anonymous methods do not have a sensible name, thus they are named lambda X/delegate X
//  where X is a running number
uint lambdaCount = 0;
uint delegateCount = 0;

public uint GetLambdaNr() {
  return lambdaCount++;
}

public uint GetDelegateNr() {
...
Stack tryResults = new Stack();
  ...
  // Creates a new block starting with the lookahead token la and adds it to the counters
  // if useStatementBlock is true, a StatementBlock is created, otherwise a CalcBlock
  //  mostly useStatementBlock is true when the previous block contained a jump (inclJump)
  BasicBlock CreateBlock(bool useStatementBlock, BasicBlock dependentBlock) {
    // last block closed correctly?
    //if( curBlock.End == null ) {
    //  throw new ApplicationException("Parsing: Block not ended: " + curBlock);
    //}
    
    if (useStatementBlock) {
      curBlock = new StatementBlock(new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col));
    } else {
      curBlock = new CalcBlock(new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col), dependentBlock);
    }
    Add(curBlock);
    return curBlock;
  }

  BasicBlock CreateBlock() { return CreateBlock(true, null); }
  BasicBlock CreateBlock(BasicBlock block) { return CreateBlock(false, block); }

  // ends the current block, fully including the current token t
  //  except for "negative" ranges, where then an empty block (Start=End and right before t)
  //  is created
  BasicBlock EndBlock() {
    if( curBlock == null ) {
      throw new InvalidOperationException("Error (ATG): Tried to end a block when none was created/started.");
    }
    
    // would be good to have this uncommented, but would require massive grammar cleanup
    //  (correct balancing of CreatedBlock and EndBlock
    //if( curBlock.End != null ) {
    //  throw new InvalidOperationException("Error (ATG): Tried to end a block a second time.");
    //}

    // don't override end position
    if (curBlock != null && curBlock.End == null) {
      // prevent "negative" blocks => create empty before the current token t
      if( curBlock.Start.AbsByChars > t.posByChars ) {
        curBlock.End = curBlock.Start;
      }
      else {
        // TODO: byte and character positions do not match!
        curBlock.End = new Position(
          (t.posByChars + t.val.Length),
          charInsertCount,
          t.pos, 
          t.line,
          t.col);
  ...
  // A "method block" with the corresponding positions here is only comprised of
  //  the method head, eg. "public MyMethod( int i )" and does NOT include the whole
  //  body. This is reflected during runtime that only the header is highlighted when
  //  this block is selected
  void CreateMethodBlock() {
    curMethod = new MethodBlock(
      new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col ));
    curBlock = curMethod;
  }
  
  MethodBlock EndMethodBlock() {
    return EndMethodBlock("");
  }
  
  MethodBlock EndMethodBlock(string template) {
    var currentMethodBlock = (MethodBlock)curBlock;
    
    // TODO: this is wrong with utf8 tokens (length returns number of characters, not bytes!
    //        -> change whole GetString to handle characters (phew!)
    currentMethodBlock.Name = 
      template + scanner.buffer.GetString(curBlock.Start.AbsByBytes, t.pos+t.val.Length);
      
    return currentMethodBlock;
  }
  
  void StartMethod(MethodBlock  method ) {
    curClass.AddMethod(method);
    counters.AddMethod(method);
    curMethod = (MethodBlock)EndBlock();
  }

  void CreateClassCounter() {
    curClass = new ClassCounter(new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col ));
    curBlock = curClass;
  }
  
  void EndClassCounter(string id) {
    curClass.Namespace = fullCurrentNamespaceName;
    curClass.Name = id;
  ...
  void SetStart() {
    curBlock.Start = new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col);
  }
  
  void SetBodyStart() {
    curClass.BodyStart = new Position(la.posByChars, charInsertCount, la.pos, la.line, la.col);
  }

  string storedTemplate = "";
  
  void StoreTemplate() {
    // eg. "public string PropertyToken.", to be later appended by "get", "set", "add" or "remove"
    //  thus forming eg. "public string PropertyToken.get" as full displayed name
    // TODO: this is wrong with utf8 tokens (length returns number of characters, not bytes!
    //        -> change whole GetString to handle characters (phew!)
    storedTemplate = scanner.buffer.GetString(curBlock.Start.AbsByBytes, t.pos + t.val.Length) + ".";
  }

  /// inserts the semantic-preserving element "value" after immediately after 
  ///   the end of the current token during instrumentation
  void AddSyntaxElement( string  value ) {
    // possibly add surrounding whitespace?
    counters.AddSyntaxElement( la.posByChars, value );
    charInsertCount += value.Length;
  }
  
  // DOC: see CounterCollection.class
  void ForceCounterStatementHere() {
    counters.ForceCounterStatementHere( la.posByChars );
  }

  void ForceCounterExpressionHere() {
    counters.ForceCounterExpressionHere( la.posByChars );
  }
  
  void ForceNewCounterIDHere() {
    counters.ForceNewCounterIDHere( la.posByChars );
  }

  // for unused parameters
  private static bool unused = false;
  #endregion
 ...
 *------------------------------------------------------------------------*/
	...
	void NamespaceMemberDeclaration() {
		...
		/* may be nested, required for unwinding */
		string  oldNamespaceName = fullCurrentNamespaceName;
		
		if (la.kind == 45) {
			Get();
			Expect(1);
			fullCurrentNamespaceName += t.val; 
			while (la.kind == 91) {
				Get();
				Expect(1);
				fullCurrentNamespaceName += "." + t.val; 
			...
			fullCurrentNamespaceName = oldNamespaceName; 
			Expect(113);
			if (la.kind == 116) {
				Get();
			}
	...
	void ClassLevelBlock() {
		Expect(97);
		if (curMethod.Ref == null) { 
		curMethod.Ref = CreateBlock(); 
		} else { CreateBlock(); }
		while (StartOf(17)) {
			Statement(out unused);
		}
		EndBlock(); 
		Expect(113);
	}
	...
	void Expression() {
		...
		if (la.kind == 123) {
			...
			MethodBlock  surrMethod = curMethod; 
			BasicBlock surrBlock = curBlock; 
			CreateMethodBlock(); 
			Expect(99);
			if (StartOf(18)) {
			...
			Expect(115);
			Expect(85);
			var mb = EndMethodBlock(surrMethod.Name + " |Lambda " + GetLambdaNr() + "| " ); 
			StartMethod(mb); 
			if (la.kind == 97) {
				ClassLevelBlock();
			} else if (StartOf(19)) {
				BasicBlock lambdaBody = CreateBlock(); 
				Debug.Assert( curMethod.Ref == null ); 
				curMethod.Ref = lambdaBody; 
				AddSyntaxElement( InstrumentationUtils.LAMBDA_HELPER_FUNC_NAME ); 
				AddSyntaxElement("("); 
				ForceNewCounterIDHere(); 
				AddSyntaxElement(", () => "); 
				Expression();
				EndBlock(); 
				AddSyntaxElement(")"); 
			} else SynErr(168);
			curMethod = surrMethod;
			curBlock = surrBlock; 
		} else if (StartOf(20)) {
			Unary();
	...
	void EventAccessorDeclarations() {
		...
		var mb = EndMethodBlock(storedTemplate); 
		StartMethod(mb); 
		ClassLevelBlock();
		if (la.kind == 1 || la.kind == 98) {
			while (la.kind == 98) {
			...
			mb = EndMethodBlock(storedTemplate); 
			StartMethod(mb); 
			ClassLevelBlock();
		}
	}
...
!firstExpr[kindInContext(Peek(1).kind)]) {
	...
	void Statement(out bool inclJump) {
	...
	void EmbeddedStatement(out bool inclJump) {
		...
		if (la.kind == 97) {
			...
			EndBlock(); 
			Expect(115);
			AddSyntaxElement("{"); BasicBlock bIfEm = CreateBlock(); 
			EmbeddedStatement(out inclJump);
			EndBlock(); AddSyntaxElement("}"); 
			if (la.kind == 24) {
				Get();
				BasicBlock bElseEm = CreateBlock(bBeforeIf - bIfEm); 
				EmbeddedStatement(out inclJump);
				EndBlock(); 
			}
			...
			BasicBlock bBeforeSwitch = EndBlock(); 
			Get();
			Expect(99);
			...
			BasicBlock bBeforeWhile = EndBlock(); 
			Get();
			Expect(99);
			BasicBlock bWhile = CreateBlock(null); AddStatement(); 
			Expression();
			EndBlock(); 
			Expect(115);
			AddSyntaxElement("{"); BasicBlock bWhileEm = CreateBlock(); 
			...
			BasicBlock bBeforeDo = EndBlock(); 
			Get();
			AddSyntaxElement("{"); BasicBlock bDoEm = CreateBlock(); 
			EmbeddedStatement(out inclJump);
			EndBlock(); AddSyntaxElement("}"); 
			Expect(82);
			Expect(99);
			...
			BasicBlock bBeforeFor = EndBlock(); 
			Get();
			Expect(99);
			if (StartOf(26)) {
			...
			Expect(115);
			AddSyntaxElement("{"); BasicBlock forEm = CreateBlock();  
			...
			BasicBlock bBeforeFe = EndBlock(); 
			Get();
			Expect(99);
			BasicBlock bFe = CreateBlock(bBeforeFe); 
			LocalVariableType();
			Expect(1);
			Expect(38);
			Expression();
			AddStatement(); EndBlock(); 
			Expect(115);
			AddSyntaxElement("{"); BasicBlock bFeEm = CreateBlock();   
			EmbeddedStatement(out inclJump);
			EndBlock(); AddSyntaxElement("}");   
			...
			Get();
			Expect(116);
			inclJump = true; AddStatement(); EndBlock(); CreateBlock();  
		} else if (la.kind == 18) {
			Get();
			Expect(116);
			inclJump = true; AddStatement(); EndBlock(); CreateBlock();  
			...
			bool blockJump = false; BasicBlock bBeforeTry = EndBlock(); 
			Get();
			BasicBlock bTry = CreateBlock(bBeforeTry); 
			Block(out blockJump);
			...
			CreateBlock(inclJump,bBeforeTry); 
		} else if (la.kind == 43) {
			Get();
			Expect(99);
			Expression();
	...
	void Block(out bool inclJump) {
		bool stmtInclJump = false; inclJump = false; 
		Expect(97);
		if (new Position(t.posByChars, charInsertCount, t.pos, t.line, t.col).Equals(curBlock.Start)) SetStart(); 
		while (StartOf(17)) {
			Statement(out stmtInclJump);
			inclJump = inclJump || stmtInclJump; 
			stmtInclJump = false; 
		}
		EndBlock();
		Expect(113);
	...
	void CatchClauses(out bool inclJump) {
	...
	void Primary() {
		...
		case 21: {
			...
			var mb = EndMethodBlock(surrMethod.Name + " |Delegate " + GetDelegateNr() + "| " ); 
			StartMethod(mb); 
			ClassLevelBlock();
			curMethod = surrMethod; 
			curBlock = surrBlock; 
