...
public:
   ...
   ReturnT notify(const ParamT& param) const
   {
...
class CppEvent1
{
   typedef std::map<int, typename DefSmartPtr<EventHandlerBase1<ReturnT, ParamT> >::Ptr> HandlersMap;
...
public:
   ...
   CppEventHandler attach(ListenerT* object, ReturnT (ListenerT::*member)(const ParamT&))
   {
      typedef ReturnT (ListenerT::*PtrMember)(const ParamT&);	
   ...
   ReturnT notify(const ParamT& param) const
   {
      HandlersMap::const_iterator it = m_handlers.begin();
